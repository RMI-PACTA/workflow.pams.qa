---
title: "Status Report - Comparison of PAMS and Banks Financial Control"
author: "Jacob Kastl"
date: \today
output: 
  html_document:
...
# set configs
pams_path <- config$compare_pams_banks_fc$paths$path_pams
banks_fc_path <- config$compare_pams_banks_fc$paths$path_banks_fc

threshold_rows_replicated <- config$compare_pams_banks_fc$thresholds$share_rows_replicated
relative_tolerance_production <- config$compare_pams_banks_fc$thresholds$relative_tolerance_production
relative_tolerance_emission_factor <- config$compare_pams_banks_fc$thresholds$relative_tolerance_emission_factor
...
This report checks if the PAMS and Banks Financial Control (FC) datasets of a common data release are consistent. For that, it compares two datasets: `pams` and `banks_fc` to ensure data consistency.

In general, it is expected that the Banks Financial Control dataset can be recreated from the PAMS dataset. Beyond minor differences due to rounding, we would expect that it is possible to derive the Banks FC dataset from PAMS after several steps of data wrangling.

We will check if the above expectation holds and gather any incosnsistencies and unexpected patterns.

## Load Data

The PAMS dataset of the 2023Q2 data release can use the import function `pacta.data.preparation::import_ar_advanced_company_indicators()`, which will return a tidy combination of the sheets contained in the PAMS xlsx. The Banks FC file only requires loading one sheet from an xlsx.

```{r import, echo = FALSE}
comparison_pams <- pacta.data.preparation::import_ar_advanced_company_indicators(pams_path, fix_names = TRUE)
comparison_banks_fc <- readxl::read_xlsx(banks_fc_path, sheet = "Company Indicators - PACTA Comp")
```
...
# Overview of banks_fc
cat("Summary of banks_fc:\n")
print(head(comparison_banks_fc))

```

## Data Preparation
We will now wrangle the PAMS dataset to match the Banks FC dataset as closely as possible.

### Step 1: Correct columns and format
We keep and rename the relevant columns from PAMS needed for Banks FC. And we add a column for the `ald_timestamp`, which is not given in PAMS.

``` {r columns_and_format, echo = FALSE, include=FALSE}
ald_timestamp <- unique(comparison_banks_fc$ald_timestamp)
if (length(ald_timestamp) > 1) {
  stop("Cannot derive the appropriate value for `ald_timestamp` from more than one value.")
}

pams_to_banks_fc <- comparison_pams %>%
  dplyr::select(
    dplyr::all_of(
      c(
        "company_id",
  ...
  dplyr::filter(
    .data$consolidation_method == "Financial Control"
  ) %>%
...
- We only keep the years that are given in the Banks FC dataset.
...
banks_fc_years <- unique(comparison_banks_fc$year)

pams_to_banks_fc <- pams_to_banks_fc %>%
  dplyr::mutate(
  ...
  dplyr::filter(.data$year %in% .env$banks_fc_years) %>% 
...
pams_to_banks_fc_production <- pams_to_banks_fc %>%
...
pams_to_banks_fc_emission_intensity <- pams_to_banks_fc %>%
...
pams_to_banks_fc_merged <- pams_to_banks_fc_production %>%
  dplyr::full_join(
    pams_to_banks_fc_emission_intensity,
...
pams_to_banks_fc_merged <- pams_to_banks_fc_merged %>%
  dplyr::mutate(
...
pams_to_banks_fc_aggregated <- pams_to_banks_fc_merged %>%
  dplyr::summarise(
...
### Step 6: Accomodate Banks FC particularieties

The Banks FC data set has a few less than obvious traits that need to be accounted for when replicating it from PAMS:
...
pams_to_banks_fc_complete <- pams_to_banks_fc_aggregated %>%
  dplyr::mutate(
    emission_factor = dplyr::if_else(
...
Finally, we compare how closely the replicated data set matches the Banks FC dataset we received.

We compare across the following dimensions:

- How many rows are successfully replicated?
- How many rows are given in the PAMS-based replica, but not in the Banks FC dataset?
- How many rows are given in the Banks FC dataset, but not in the PAMS-based replica?
- What are the patterns of those rows that were not successfully replicated?
- Are the production and emission_factor values equal between the successfully replicated rows and the Banks FC dataset?
...
common_data <- comparison_banks_fc %>%
  dplyr::inner_join(
    pams_to_banks_fc_complete,
    by = c(
...
share_rows_replicated <- nrow(common_data) / nrow(comparison_banks_fc)

if (share_rows_replicated < threshold_rows_replicated) {
    warning(glue::glue("The share of rows from the Banks FC dataset that was successfully replicated, is {round(share_rows_replicated * 100, 3)}%, which is less than the expected minimum threshold of {threshold_rows_replicated * 100}%."))
}
  
if (share_rows_replicated >= threshold_rows_replicated) {
  message(glue::glue("The share of rows from the Banks FC dataset that was successfully replicated, is {round(share_rows_replicated * 100, 3)}%."))
}
...
We now compare the sectoral production totals and weighted average emission_factor from the Banks FC and the PAMS-based replica.

```{r compare_total, echo=FALSE}

totals_banks_fc <- comparison_banks_fc %>% 
  dplyr::summarise(
    emission_factor = stats::weighted.mean(.data$emission_factor, w = .data$production, na.rm = TRUE),
    production = sum(.data$production, na.rm = TRUE),
    .by = c("year", "sector")
...
totals_pams_replica <- pams_to_banks_fc_complete %>% 
  dplyr::summarise(
    emission_factor = stats::weighted.mean(.data$emission_factor, w = .data$production, na.rm = TRUE),
    production = sum(.data$production, na.rm = TRUE),
    .by = c("year", "sector")
...
for (i in unique(totals_banks_fc$sector)) {
  ...
  if (is.na(sector_totals_i)) {
    ...
    warning(glue::glue("In the {i} Sector, there are differences in total production values between the Banks FC and the PAMS replica data sets.}"))
  } else if (sector_totals_i) {
    message(glue::glue("In the {i} Sector, total production values are identical between the Banks FC and the PAMS replica data sets."))
  }
}
...
for (i in unique(totals_banks_fc$sector)) {
  ...
  if (is.na(sector_totals_i)) {
    ...
    warning(glue::glue("In the {i} Sector, there are differences in weighted mean `emission_factor` values between the Banks FC and the PAMS replica data sets.}"))
  } else if (sector_totals_i) {
    message(glue::glue("In the {i} Sector, weighted mean `emission_factor` values are identical between the Banks FC and the PAMS replica data sets."))
  }
}
...
Below, we see a table of cases that shows rows that could not be recreated from PAMS, although they are given in Banks FC. This may explain to some degree the differences between production matches in the replicated data. In any case, these differences may need explanation/investigation from AI.

```{r distinct_data_banks, echo=FALSE}
distinct_data_banks <- comparison_banks_fc %>%
  dplyr::anti_join(
    pams_to_banks_fc_complete,
